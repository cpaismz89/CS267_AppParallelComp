...
kmer_pair::kmer_pair(const kmer_pair &kmer) {
  init(kmer);
}

kmer_pair kmer_pair::operator=(const kmer_pair &kmer) {
  init(kmer);
}
...
void kmer_pair::init(const kmer_pair &kmer) {
  this->kmer = kmer.kmer;
  for (int i = 0; i < 2; i++) {
